...
SemaphoreHandle_t g_is_conversion_done;
...
static void AFEC_pot_Callback(void){
	...
	xSemaphoreGiveFromISR(g_is_conversion_done, pdFALSE);
}
...
void task_adc(void) {
	...
	if (g_is_conversion_done == NULL) printf("falha em criar o semaforo \n");

	while(1) {
		if (xSemaphoreTake(g_is_conversion_done, ( TickType_t ) 500) == pdTRUE) {
			printf("%d\n", g_ul_value);
