...
int process_touch(t_but botoes[], touchData touch, uint32_t n) {
	for (int i = 0; i < n; i++) {
		t_but but = botoes[i];
		int x1 = but.x - but.width / 2;
		int x2 = x1 + but.width;
		int y1 = but.y - but.height / 2;
		int y2 = y1 + but.height;
		
		if (touch.x >= x1 && touch.x <= x2 && touch.y >= y1 && touch.y <= y2) return i;
	}
	return -1;
}
...
void task_lcd() {
	...
	draw_button_new(but0, 1);
	draw_button_new(but1, 1);
	draw_button_new(but2, 1);
	...
	while (1) {
		if (xQueueReceive( xQueueTouch, &(touch), ( TickType_t )  500 / portTICK_PERIOD_MS)) {
			...
			draw_button_new(but1, buts_status[1]);
			
			draw_button_new(but2, buts_status[2]);
			printf("x:%d y:%d\n", touch.x, touch.y);
		}
	}
}
